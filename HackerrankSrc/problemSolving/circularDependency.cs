...
public static class DependencyExtensions
{
    ...
    {
        var state = ValueOrDefault(visited, node, VisitState.NotVisited);
    ...
    public static List<List<T>> FindCycles<T>(IEnumerable<T> nodes, Func<T, IEnumerable<T>> edges)
    {
        var cycles = new List<List<T>>();
    ...
    public static List<List<T>> FindCycles<T, TValueList>(IDictionary<T, TValueList> listDictionary)
        where TValueList : class, IEnumerable<T>
    {
        return FindCycles(listDictionary.Keys, key => ValueOrDefault(listDictionary, key, null) ?? Enumerable.Empty<T>());
    }
    ...
    public static void Main()
    {
        ...
        var cycles = FindCycles(serviceDependence);
        ...
        Debug.Assert(FindCycles(serviceDependence).Count == 0);
    }
}