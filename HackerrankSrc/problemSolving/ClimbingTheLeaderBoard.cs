...
class Solution
{
    ...
    static int[] climbingLeaderboard(int[] scores, int[] alice)
    {
        ...
        rankPos = GetRank(newScores, alice[0], newScores.Length);
        aliceRanks[0] = rankPos[0];

        //List<int> newSc = newScores.ToList();
        int prevPos = rankPos[1];
        int prevRank = rankPos[0];
        for (int i = 1; i < alice.Length; i++)
        {
            
            newScores[prevPos] = -1;

            rankPos = Climb(newScores, prevPos, alice[i], prevRank);
    ...
    static int[] Climb(int[] newScores, int prevPos, int currentScore, int prevRank)
    {
        int newPos = 0;
        int rank = prevRank;
        ...
        return GetRank(newScores, currentScore, newPos);
        ...
        //return new int[2] { rank, newPos };
        //return newScores;
    }



    static int[] GetRank(int[] a, int aliceScore, int max)
    {
        int rank = 1;
        int position = -1;
        int prev = a[0];
